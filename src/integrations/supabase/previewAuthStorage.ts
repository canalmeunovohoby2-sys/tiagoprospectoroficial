...
// postMessage so the project's preview surfaces share one login; else safeLocal.
export function brokeredPreviewStorage() {
  ...
  if (!projectId || !framed) return safeLocalStorage as unknown as Storage;
  ...
  return {
    getItem: async (key: string) => {
      ...
      if (res && res.ok && typeof res.value === 'string') {
        if (res.value === '') { safeLocalStorage.removeItem(key); return null; }
        return res.value;
      }
